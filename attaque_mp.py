...
def attaque_mp(couple1, couple2):
	'''
	processes = []
	for _ in range(4):
		p = multiprocessing.Process(target = attaque, args=[])
		p.start()
		processes.append(p)
	...
	with concurrent.futures.ProcessPoolExecutor() as executor:
		results = executor.map(generation_listes, <args>, [x*(CHKSIZE) for x in range(NBR_PROCESS)])
		collisions = []
		for lmlcs in results:
			collisions += trouver_collision(lmlcs)
		results += executor.map(tests_cle, <args>, [x*(len(collisions)//NBR_PROCESS) for x in range(NBR_PROCESS)])
